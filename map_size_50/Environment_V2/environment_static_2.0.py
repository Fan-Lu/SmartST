...
class environment:
    ...
    def time_punishment(self, location):  # calculate the time punishment
        speed = self.maps[self.map_index][0][location]
    ...
    def moveable_list(self):
        ...
        i = 0
        if self.way_back and self.last_loc:
            for ele in self.action_space_name:
                temp_loc = self.current_loc + np.array(self.action_space[ele])
                if temp_loc != self.last_loc and temp_loc[0]<=self.map_size-1 and temp_loc[0]>=0 and \
                        temp_loc[1]<=self.map_size-1 and temp_loc[1]>=0 and tmp[temp_loc] != 0:
                    r_l[i] = 1
                i += 1
        else:
            for ele in self.action_space_name:
    ...
    def reset(self, map_number=None, game_number=None, plot=False):
        ...
        if plot:
            ...
            tmp[self.current_game_start] = 255
            tmp[self.current_game_target] = 255
