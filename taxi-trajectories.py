...
full_files = get_files('release')          # All folders  700 MB
...
grouped = data.sort_values(by=['date_time']).groupby('taxi_id')['date_time']
...
grouped = data.sort_values(by=['date_time']).groupby('taxi_id')
...
for g in grouped:
    ...
    distances.append(pd.DataFrame(pythagoras(g[1].latitude.values,g[1].longitude.values))) # unit:m
...
window = data[(xmin < data.longitude) & (data.longitude < xmax) & (ymin < data.latitude) & (data.latitude < ymax)] # filter data
